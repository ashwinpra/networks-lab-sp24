...
char* encrypt_text(char* text, int k){
...
int main() {
    ...
    while(1) {
        ...
        if (fork() == 0) {
            ...
            char text[FILE_ACCESS_SIZE];
            while((n = read(fd, text, FILE_ACCESS_SIZE)) > 0) {
                write(enc_fd, encrypt_text(text, k), n);
            }
            ...
            char enc_text[FILE_ACCESS_SIZE];
            while((n = read(fd, enc_text, FILE_ACCESS_SIZE)) > 0) {
                send(newsockfd, enc_text, n, 0);
            }
